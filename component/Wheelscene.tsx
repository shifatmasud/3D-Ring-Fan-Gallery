...
/**
 * A truly robust, shallow comparison for the items array. This is safer and more
 * performant than JSON.stringify and prevents crashes from undefined data.
 * @param a The first array of items.
 * @param b The second array of items.
 * @returns boolean True if the items are considered equal.
 */
const itemsAreEqual = (a: Item[], b: Item[]): boolean => {
    if (a === b) return true;
    // This is the critical guard. It prevents the `a.length` or `b.length`
    // access below from crashing if a or b is null or undefined.
    if (!a || !b) return false;
    if (a.length !== b.length) return false;
    for (let i = 0; i < a.length; i++) {
        // A simple check on key properties is usually sufficient and performant.
